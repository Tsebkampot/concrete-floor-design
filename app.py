...
def calculate_project_results(params: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"slab": slab, "secondary": secondary, "main": main, "transfer": transfer, "matrix": matrix_project}
...
def render_home_dashboard(params: dict[str, Any], tables: dict[str, pd.DataFrame], results: dict[str, Any]) -> None:
    ...
    with c2:
        st.download_button(
            "Word 计算书",
            build_word_report("整体式单向板肋形楼盖半自动计算书", tables, notes, home_images),
...
def render_export_page(tables: dict[str, pd.DataFrame], results: dict[str, Any]) -> None:
    render_header("结果导出与半自动计算书", "一键生成课程设计答辩和计算书整理所需成果文件。", ["Excel", "Word", "PNG"])
    notes = [
        "当前支持 Word/Excel/PNG 导出，PDF 可由 Word 另存为 PDF。",
        "正式内力包络采用矩阵刚度法；斜截面箍筋、最小配筋和构造要求仍需人工复核。",
    ...
    with c2:
        st.download_button(
            "导出 Word 半自动计算书",
            build_word_report("整体式单向板肋形楼盖半自动计算书", tables, notes, images),
            file_name="整体式单向板肋形楼盖半自动计算书.docx",
            mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
        )
    with c3:
    ...
    render_info_box("控制截面示意图、矩阵弯矩包络图和剪力包络图已嵌入 Word/Excel，也可在各构件页面单独导出 PNG。")
...
def main() -> None:
    ...
    if page == "🏠 首页":
        ...
        render_export_page(tables, results)
    elif page == "📄 自动计算书":
