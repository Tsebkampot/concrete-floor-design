...
- 结果导出：支持完整 Excel、汇总版 Word/Markdown/PDF 计算书和各类 PNG 图片导出。
...
7. 在“结果导出与半自动计算书”导出完整 Excel，或导出按“计算数据总表”口径整理的 Word/Markdown/PDF 计算书。
8. 各图形页面可单独导出 PNG。
...
确认首页可以打开，板、次梁、主梁基础计算页正常显示，Excel、Word、Markdown、PDF 和 PNG 导出按钮可以生成下载文件。
...
- 自动计算书太长：Word、Markdown 和 PDF 计算书默认只输出“计算数据总表、复核说明、主梁补充数据”；完整矩阵明细请查看 Excel 导出。
